  ...
  return (
    <section className="section-padding bg-surface">
      <div className="max-w-[1400px] mx-auto px-5 sm:px-8 lg:px-12">
        ...
        {sectionTitle && (
          <AnimatedSection>
            <SectionHeading align="left" title={sectionTitle} />
          </AnimatedSection>
        )}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4 md:gap-5">
          {CLIENT_CASES.map((item, index) => (
            <motion.article
              ...
              className="group overflow-hidden rounded-xl sm:rounded-2xl bg-surface-elevated/30 border border-white/[0.04] hover:border-gold/20 transition-all duration-700 hover:gold-glow"
            >
              {showImages && "image" in item && item.image ? (
